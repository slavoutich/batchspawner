...
class BatchSpawnerBase(Spawner):
    ...
    def cancel_batch_job(self, force=False):
        subvars = self.get_req_subvars()
        subvars['job_id'] = self.job_id
        if force and self.batch_force_cancel_cmd:
            cmd = self.batch_force_cancel_cmd.format(**subvars)
        else:
...
""").tag(config=True)
    ...
    batch_cancel_cmd = Unicode(
        'sudo -E -u {username} scancel --signal=INT {job_id}').tag(config=True)
    batch_force_cancel_cmd = Unicode(
        'sudo -E -u {username} scancel {job_id}').tag(config=True)
